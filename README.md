...
- 모델 비교("~이었다면")는 세션 전체 토큰을 그 모델 하나로 환산한 값이고, 옆의 차액은 실제로 낸 값과의 차다. 실제로 낸 값은 호출마다 그때 쓴 모델의 단가로 쌓이므로, 한 세션에서 모델을 바꿔 썼다면 두 값의 기준이 달라 차액이 정확하지 않다. 지금 쓰는 모델은 목록에서 빠진다.
