...
| 사용자 육안 | `node server.js --dir <대상> --port <빈 포트>`로 띄운 대시보드를 브라우저에서 본다. 포트를 고정하지 않는다(7777은 사용자의 상시 모니터가 점유하는 일이 잦다). 쓴 포트 번호를 do §3에 적는다. 카드 · 그래프 · Cache breaks 표 · 습관 순위 · 캐릭터 상태 | 사용자가 확인한 날짜를 do §3에 남긴다. 확인이 끝나면 그 서버를 종료하고, 종료했다는 사실을 do §3에 남긴다. 사용자의 상시 모니터(포트 7777, 실행 경로 `~/app/cc-monitor/server.js`)는 어떤 경우에도 종료하지 않는다. 재현에 외부 조건(세션 여러 개 · 유발할 사건 · 대기 시간 · 별도 브라우저 프로필)이 필요하면 plan §5에 그 조건을 SC별로 적는다 |
